...
$query = $conn->query("SELECT `p`.`id`, `p`.`folderid`, `p`.`name`, `p`.`description`, `p`.`latitude`, `p`.`longitude`, `p`.`altitudecapability`, `p`.`time`, `p`.`srt`, `p`.`common`, `p`.`userid` FROM `places` `p` WHERE `p`.`userid` = '" . $_GET["id"] . "' OR `p`.`common` = 1");
