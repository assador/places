...
$query = $conn->query("SELECT `p`.`id`, `p`.`folderid`, `p`.`name`, `p`.`description`, `p`.`link`, `p`.`latitude`, `p`.`longitude`, `p`.`altitudecapability`, `p`.`time`, `p`.`srt`, `p`.`geomark`, `p`.`common`, `p`.`userid` FROM `places` `p` WHERE `p`.`userid` = '" . $_GET["id"] . "' OR `p`.`common` = 1");
...
$query = $conn->query("SELECT `f`.`id`, `f`.`parent`, `f`.`name`, `f`.`description`, `f`.`srt`, `f`.`geomarks` FROM `folders` `f` WHERE `f`.`userid` = '" . $_GET["id"] . "' ORDER BY `srt`");
