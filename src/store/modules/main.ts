...
export const mutations: MutationTree<IMainState> = {
	...
	deleteFolder(state, payload) {
		if (!payload.source) {
			payload.source = getters.treeFlat[payload.folder.parent];
		}
...
export const actions: ActionTree<IMainState, State> = {
	...
	async setPlaces({state, commit, dispatch, getters}, payload) {
		...
		*/
		...
		function parseGPX(store, text: string) {
			const result = {
		...
		switch (payload.mime) {
			case 'application/json' :
				...
				break;
			case 'application/gpx+xml' :
				parsed = parseGPX(store, payload.text);
				break;
			default :
				dispatch('setMessage',
		...
		addImported(store, payload.mime, parsed);
	},
...
export const getters: GetterTree<IMainState, State> = {
	placeFields(state) {
		const placeFields = {
...
export const main: Module<IMainState, State> = {
	namespaced,
	state,
	getters,
	actions,
