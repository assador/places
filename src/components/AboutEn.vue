...
<li><a href="#2-system-concept">System Concept</a><ul>
...
<li><a href="#left-side">Left side</a></li>
...
<li><a href="#6-basic-workflow">Basic Workflow</a><ul>
...
<p>A Point consists of specific geographic coordinates: latitude, longitude, and altitude. Plus its own unique identifier — an ID.</p>
...
<strong>One Point — one geography. It can be reused multiple times.</strong></p>
<h3>Place</h3>
<p><strong>Semantic description of a Point.</strong> It contains: a name, text description, images, and a link to one specific Point. Plus a lot of other additional optional information you might want to add.</p>
<p>When a Place is created, the system automatically creates a new Point and &quot;links&quot; this Place to it. This technical detail is hidden from the user to simplify the interface. The <strong>Place itself does not store geographic coordinates</strong>. It is the meaning, the meta-description of a specific location in your life, which naturally has its own geography (the Point).</p>
<p><strong>Example</strong>:
&quot;The gazebo where we first kissed Masha.&quot;
You can save this place (already with a Point on the map), add a description and photos, and then include it in a &quot;nostalgic evening stroll&quot; route. Technically, when you include a Place in a route, it is the associated Point that is added, not the Place itself.</p>
<h3>Route</h3>
...
You start from home (the Point linked to the &quot;My Home&quot; Place), go to a shop (Point), then a library (Point), wander through the park, and on the way back stop by the same library again (the same Point) before returning home.</p>
<h3>Folder</h3>
...
<p>Technically, <strong>a Folder does not &quot;contain&quot; other Folders</strong>. The entire hierarchy is built on the &quot;parent link&quot; principle. Each folder has a Parent ID. The same applies to Places and Routes. A regular user doesn't need to worry about this.</p>
<h3>Images</h3>
...
<li><strong>PWA / Install</strong>: Button to install “The Places” as a standalone application on your device.</li>
<li><strong>Export / Import</strong>: Tools for working with external data formats (<strong>GPX</strong>, <strong>JSON</strong>).</li>
...
<h3>Bottom Panel (Footer)</h3>
<h4>Left side</h4>
<p><strong>Show Own / Show All</strong>: Independent toggle buttons to show only your markers/routes or all public data available in the system.</p>
<h4>Right side</h4>
<ul>
...
<p>For example: you press the “leader key” → a blinking indicator lights up in the top left corner of the window (indicating you are in shortcut mode) → pressing <code>A</code> next adds a new place and exits shortcut mode.</p>
...
<h2>5. Storage and Ordering Principles</h2>
...
<p><strong>Explicit Saving</strong>: An &quot;offline-first&quot; / &quot;deferred saving&quot; model. Changes are marked as &quot;dirty&quot; and sent to the server only when the &quot;Save&quot; button is clicked. This provides control over transactions and allows for batch updates.</p>
<h2>6. Basic Workflow</h2>
<h3>Creating a Place</h3>
<ol>
...
<h2>7. Architectural Features</h2>
<ul>
...
<h3>Deletion and Data Safety</h3>
