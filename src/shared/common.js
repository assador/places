...
export const sortObjects = (array, field) => {
...
export const num2deg = (num, lat = false) => {
    const n = num % 360;
    if (Math.abs(n) <= (lat ? 90 : 180))
        return n;
    if (lat && Math.abs(n) < 180)
        return -n % 90 + 90 * (n < 0 ? -1 : 1);
    if (lat && Math.abs(n) < 270)
        return -n % 90;
    if (lat)
        return n % 90 + 90 * (n < 0 ? 1 : -1);
    return n % 180 + 180 * (n < 0 ? 1 : -1);
};
export const deg2degMinSec = (frac) => {
...
export const degMinSec2deg = (dms) => {
    return (Math.abs(dms[0]) + dms[1] / 60 + dms[2] / 3600) * (dms[0] < 0 ? -1 : 1);
};
export const coords2string = (coords) => {
...
export const string2coords = (coords) => {
...
export const treeToLivePlain = (tree, childrenKey, plain) => {
    plain[tree.id] = tree;
...
export const plainToTree = (plain) => {
    const tree = {};
    let ready = false;
...
export const treeToPlain = (tree, childrenKey, plain) => {
...
export const findInTree = (tree, childrenKey, key, value) => {
    if (tree[key] === value) {
        return tree;
    }
    if (tree[childrenKey]) {
        for (const sub in tree[childrenKey]) {
            ...
            resultForRecursive = findInTree(tree[childrenKey][sub], childrenKey, key, value);
...
export const isParentInTree = (tree, childrenKey, parentId, childId, parent) => {
    if (!parent) {
    ...
    for (const id in parent[childrenKey]) {
        if (isParentInTree(tree, childrenKey, parentId, childId, parent[childrenKey][id])) {
            return true;
        }
    }
    return false;
};
export const formFoldersCheckedIds = () => {
    const foldersCheckedIds = [];
    if (document.getElementById('popup-export__tree')) {
        let check = false;
        for (const folder of document.getElementById('popup-export__tree')
            .getElementsByClassName('folder')) {
            check = true;
            for (const placeCheckbox of folder.getElementsByClassName('to-export-place-checkbox')) {
            ...
            if (check) {
                foldersCheckedIds.push(folder.id.match(/^to-export-places-menu-folder-(.+)$/)[1]);
            }
        }
    }
    return foldersCheckedIds;
};
export const treeNewIds = (tree, childrenKey, parentKey, items, itemParentKey) => {
    ...
    if (Array.isArray(tree[childrenKey]) && tree[childrenKey].length > 0) {
        for (let i = 0; i < tree[childrenKey].length; i++) {
            ...
            treeNewIds(tree[childrenKey][i], childrenKey, parentKey, items, itemParentKey);
        }
    }
};
export const formFolderForImported = (voc, time, imported) => {
    /**
...
export const sortObjectsByProximity = (array) => {
    // Sort geomarks so as to build the shortest path through them.
    let indexNearest = -1, distCurrent, distMin = 10, lastIndex = 0;
    while (array.length > lastIndex + 1) {
        for (let i = lastIndex + 1; i < array.length; i++) {
            const llt = (Number(array[lastIndex].latitude) ||
                Number(constants.map.initial.latitude) ||
                null) * Math.PI / 180;
            const lln = (Number(array[lastIndex].longitude) ||
                Number(constants.map.initial.longitude) ||
                null) * Math.PI / 180;
            const clt = (Number(array[i].latitude) ||
                Number(constants.map.initial.latitude) ||
                null) * Math.PI / 180;
            const cln = (Number(array[i].longitude) ||
                Number(constants.map.initial.longitude) ||
                null) * Math.PI / 180;
            distCurrent =
                Math.acos(Math.sin(llt) * Math.sin(clt) +
...
export const childrenCount = (tree, childrenKey, result = 0) => {
...
export const changeByKeyValue = (tree, childrenKey, key, value, what) => {
    if (tree[childrenKey] && Object.keys(tree[childrenKey]).length > 0) {
        for (const id in tree[childrenKey]) {
            switch (what) {
                case "delete":
                    if (tree[childrenKey][id][key] === value) {
                        ...
                        changeByKeyValue(tree[childrenKey][id], childrenKey, key, value, what);
                    }
                    break;
                case "change":
...
export const numbersMinMax = (array, count, order) => {
...
export const distanceOnSphere = (lat1, lon1, lat2, lon2, radius) => {
    /**
    ...
    const ang = Math.acos(Math.cos(a) * Math.cos(b) + Math.sin(a) * Math.sin(b) * Math.cos(c));
