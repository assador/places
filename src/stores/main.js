...
import { generateRandomString, num2deg, treeToLivePlain, plainToTree, formFolderForImported, distanceOnSphere, } from '@/shared/common';
...
export const useMainStore = defineStore('main', {
    ...
    actions: {
        ...
        backupState() {
            if (!this.backup || this.stateBackups.length >= constants.backupscount)
                return;
            this.stateBackups.splice(++this.stateBackupsIndex);
            this.stateBackups.push(Object.assign({}, JSON.parse(JSON.stringify(this.$state))));
        ...
        deleteImages(payload) {
            ...
            for (const id in payload.images) {
                if (this.places[payload.images[id].placeid] &&
        ...
        showInRange(range) {
            ...
            for (const id in this.places) {
                if (distanceOnSphere(this.waypoints[this.places[id].waypoint].latitude, this.waypoints[this.places[id].waypoint].longitude, this.waypoints[this.currentPlace.waypoint].latitude, this.waypoints[this.currentPlace.waypoint].longitude, constants.earthRadius) > range) {
                    this.places[id].show = false;
                }
                else {
                    this.places[id].show = true;
        ...
        measureDistance() {
            ...
            if (this.measure.places.length < 2)
                return;
            for (let i = 0; i < this.measure.places.length; i++) {
                if (this.measure.places[i] === null)
                    continue;
                if (lastIdx === null) {
                    lastIdx = i;
                    continue;
                ...
                this.measure.distance += distanceOnSphere(this.waypoints[lastPlace.waypoint].latitude, this.waypoints[lastPlace.waypoint].longitude, this.waypoints[place.waypoint].latitude, this.waypoints[place.waypoint].longitude, constants.earthRadius);
                lastIdx = i;
            }
        },
        ...
        restoreState(backupIndex) {
            ...
            for (const key in this.stateBackups[backupIndex]) {
                if (key !== 'inUndoRedo' &&
                    ...
                    key !== 'treeFlat') {
                    this[key] =
                        JSON.parse(JSON.stringify(this.stateBackups[backupIndex][key]));
                }
            }
        ...
        async setUser() {
            return axios
                .get('/backend/get_account.php?id=' +
        ...
        async setServerConfig() {
            return axios
                .get('/backend/get_config.php?userid=' +
                sessionStorage.getItem('places-userid'))
                .then(response => {
        ...
        setFirstCurrentPlace() {
            ...
            if (this.homePlace)
                ...
                if (firstPlaceInRoot)
                    this.currentPlace = firstPlaceInRoot;
                else
                    this.currentPlace = this.places[Object.keys(this.places)[0]];
            }
        },
        async setPlaces(payload) {
            ...
            */
            ...
            const parseGPX = (text) => {
                ...
                let importedPlaceFolder, importedFolder;
                ...
                for (const wpt of dom.getElementsByTagName('wpt')) {
                    ...
                    */
                    importedPlaceFolder = formFolderForImported(this.t, time.slice(0, 10), importedFolder);
                            ...
                            try {
                                switch (desc.nodeType) {
                                    ...
                                    case 3:
                                        ...
                                        break;
                                    case 4:
                                        const reStr = 'desc_(?:user|test)' +
                                            '\s*\:\s*start\s*--\s*>\s*' +
                                            '(.*?)' +
                                            '\s*<\s*\!\s*--\s*' +
                                            'desc_(?:user|test)' +
                                            '\s*\:\s*end';
                                        const descs = desc.textContent.match(new RegExp(reStr, 'gi'));
                                        for (let i = 0; i < descs.length; i++) {
                                            description += descs[i].replace(new RegExp(reStr, 'i'), '$1') + (desc.nextSibling ? '\n' : '');
                                        }
                                        break;
                                }
                            }
                    ...
                    const newWaypoint = {
                        id: newWaypointId,
                        latitude: Number(wpt.getAttribute('lat')) ||
                            Number(constants.map.initial.latitude) ||
                            null,
                        longitude: Number(wpt.getAttribute('lon')) ||
                        ...
                        case 'application/json':
                            ...
                            while (!allParentsAdded) {
                                ...
                                for (const folder of parsed.folders) {
                                    ...
                                    */
                                    if (this.treeFlat[folder.id] ||
                                        !this.treeFlat[folder.parent] &&
                                            parsed.folders
                                                .find(f => f.id === folder.parent)) {
                                        continue;
                                    }
                                    if (this.serverConfig.rights.folderscount < 0 ||
                    ...
                    for (const place of parsed.places) {
                        ...
                        */
                        if (this.places[place.id])
                            continue;
                        if (this.serverConfig.rights.placescount < 0 ||
                            ...
                            if (this.waypoints[place.waypoint]) {
                                ...
                                const parsedWaypoint = parsed.waypoints
                                    .find(w => w.id === place.waypoint);
                                newWaypoint = {
                                    id: parsedWaypoint.id,
                                    latitude: Number(parsedWaypoint.latitude) ||
                                        Number(constants.map.initial.latitude) ||
                                        null,
                                    longitude: Number(parsedWaypoint.longitude) ||
                                        Number(constants.map.initial.longitude) ||
                                        null,
            ...
            switch (payload.mime) {
                ...
                case 'application/gpx+xml':
                    ...
                    break;
                default:
                    this.setMessage(this.t.m.popup.invalidImportFileType);
                    return false;
            }
            addImported(payload.mime, parsed);
        },
        async setUsers(payload) {
            let ids = null;
            switch (payload) {
                case 'common':
                    ids = [];
            ...
            return axios
                .post('/backend/get_users.php', Array.isArray(ids) ? { users: ids } : null)
                .then(response => {
        ...
        async deletePlaces(payload) {
            ...
            for (const payloadPlaceId in payload.places) {
                ...
                if (!commonWaypoint) {
                    waypoints.push(this.waypoints[payload.places[payloadPlaceId].waypoint]);
                    waypoints[waypoints.length - 1].deleted = true;
                    this.deleteWaypoint(this.waypoints[payload.places[payloadPlaceId].waypoint]);
                }
        ...
        async changePlace(payload) {
            ...
            if ('latitude' in payload.change || 'longitude' in payload.change) {
                const lat = num2deg(('latitude' in payload.change
                    ? payload.change.latitude
                    : this.waypoints[payload.place.waypoint].latitude), true), lng = num2deg(('longitude' in payload.change
        ...
        moveFolder(payload) {
            ...
            const srt = (('srt' in payload) ? payload.srt : (Object.keys(target.children).length
                ? target.children[Object.keys(target.children)[Object.keys(target.children).length - 1]].srt + 1
                : 1));
            if (target !== source) {
        ...
        showHideGeomarks(payload) {
            let visibility;
            const showHideSubGeomarks = (object, show) => {
            ...
            const showHideParentsGeomarks = (object) => {
                if (object.id === 'root')
                    return;
                const parentProperty = (object.type === 'place' ? 'folderid' : 'parent');
                let geomarksProperty;
                let neibours = Object.values(this.places).filter((neibour) => neibour.folderid === object[parentProperty]);
                if (this.treeFlat[object[parentProperty]].children) {
                    neibours = neibours.concat(Object.values(this.treeFlat[object[parentProperty]].children));
                }
        ...
        setMessage(message, freeze) {
            ...
            this.messageTimer = window.setInterval(() => {
                if (!this.mouseOverMessages && !freeze) {
                    ...
                    window.setTimeout(() => {
                        this.deleteMessage(this.messages[this.messages.length - 1]);
                    }, 500);
                }
            }, 5000);
        },
