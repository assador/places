...
7. [Офлайн-режим](#7-офлайн-режим)
	- [Офлайн-карты OpenStreetMap](#офлайн-карты-openstreetmap)
8. [Архитектурные особенности](#8-архитектурные-особенности)
...
С «Местами» можно работать полностью автономно, без доступа к Сети. Для этого предусмотрен специальный «**Офлайн-режим**», в который вы переключаетесь сами, явно. Что-то вроде режима «В самолёте» для мобильных устройств.

Также, технически независимо от самого Офлайн-режима, у вас есть текущий **статус сети** — Онлайн / Офлайн.

**Офлайн-режим** включается специальной кнопкой в шапке. Нажали на неё — она подсвечивается — вы полностью автономны. Нажали ещё раз — подсветка убирается — вы, в зависимости от доступности самой Сети, — Онлайн или Офлайн.

На самой этой кнопке в углу есть цветной кружочек, он показывает **статус сети**:  
**Серый** — включен **Офлайн-режим** (**статус сети** при этом, очевидно, Офлайн).  
**Красный** — **Офлайн-режим** выключен, но сама Сеть недоступна, **статус сети** — Офлайн.  
**Зелёный** — **Офлайн-режим** выключен, сама Сеть доступна, **статус сети** — Онлайн. Это обычный режим работы по-умолчанию.

Если ваш ручной Офлайн-режим **выключен**, приложение периодически пингует свой сервер в Сети, на котором крутятся бэкенд и база данных. Если пинг не проходит, статус сети меняется на **Офлайн**, кружочек красный (также появляется всплывашка с сообщением и т.д.). Пинги продолжаются до победного конца. Как только начинают проходить, статус сети меняется обратно на **Онлайн**, кружочек зелёный.

Если вы **включаете** Офлайн-режим, статус сети меняется на **Офлайн**, никто никого не пингует, любые обращения к Сети и, соответственно, к бэку и БД на сервере, прекращаются. Вы полностью автономны, кружочек серый. Когда вы обратно **выключите** Офлайн-режим, появится соответствующее сообщение, начнутся пинги, и, как только станут проходить, ваш статус сети станет **Онлайн**, кружочек зелёный.

Теперь о механике работы. Как написано выше в этом мануале, «Места» руководствуются принципом явного сохранения изменений, «batch saving» — любые запросы на изменения чего бы то ни было в БД на сервере через бэк происходят только по явному нажатию вами кнопки «Сохранить».

Если **статус сети** — Онлайн — все сделанные вами изменения по нажатию вами на кнопку «Сохранить» посылаются на сервер и записываются там в базу данных.

Если **статус сети** — Офлайн —  всё пишется только непосредственно на диск вашего устройства и никуда больше. Как только **статус сети** станет Онлайн — вы сможете ещё раз нажать на кнопку «Сохранить» — и только тогда все ваши накопленные на диске устройства изменения улетят на сервер в базу данных.

### Офлайн-карты OpenStreetMap

Специально для **Офлайн-режима** (или просто если Сеть недоступна, и **статус сети** — Офлайн), в «Местах» предусмотрена возможность импортировать тайлы для карт OpenStreetMap из ZIP-архива, заранее где-нибудь вами раздобытого. Кнопочка для этого расположена сразу справа от выпадающего списка выбора карты — OpenStreetMap / Яндекс.Карты. Но только если выбрана OpenStreetMap.

Таким образом, импортировав на своё устройство карты из ZIP-архива и переключившись в **Офлайн-режим**, вы можете работать абсолютно автономно, без доступа к Сети.
